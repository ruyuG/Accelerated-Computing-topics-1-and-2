...
def one_energy_vectorized(arr, ix, iy, nmax):
    ...
    # Compute angles between the cell and its neighbors
    angles = arr[ix, iy] - arr[[ixp, ixm, ix, ix], [iy, iy, iyp, iym]]

    # Calculate the energy contribution from all neighbors at once
    en = np.sum(0.5 * (1.0 - 3.0 * np.cos(angles) ** 2))
    return en
